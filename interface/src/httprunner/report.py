...
import requests
from src.httprunner import loader, logger
from src.httprunner.__about__ import __version__
from src.httprunner.compat import basestring, bytes, json, numeric_types
...
def render_html_report(summary, report_template=None, report_dir=None):
    ...
    today_report_dir = report_dir + os.path.sep + datetime.now().strftime("%Y%m%d")
    print(today_report_dir)
    if not os.path.isdir(today_report_dir):
    ...
    file_name = datetime.now().strftime("%Y%m%d%H%M%S.%f")[0:18] + ".html"
    report_path = today_report_dir + os.path.sep + file_name
