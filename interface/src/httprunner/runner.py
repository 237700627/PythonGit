...
from src.httprunner import exceptions, logger, response, utils
from src.httprunner.client import HttpSession
from src.httprunner.context import SessionContext
from src.httprunner.compat import OrderedDict
import time
...
class Runner(object):
    ...
    def _run_test(self, test_dict):
        ...
        if test_dict.get('pre_wait'):
            print('开始等待时间')
            time.sleep(test_dict.get('pre_wait'))
            print('结束等待时间')
        # request
        ...
        print('extractors is ...',extractors)
        response_extractors = {}
        test_variables_extractors ={}

        for key,value in extractors.items():

            if value.startswith('variables'):
                test_variables_extractors[key] = value
            else:
                # if value.startswith('content') or value.startswith('json'):
                    response_extractors[key] = value

        print(response_extractors)
        extracted_variables_mapping = resp_obj.extract_response(response_extractors)
        self.session_context.update_session_variables(extracted_variables_mapping)
        if test_variables_extractors:
            extracted_variable_mapping = OrderedDict()
            for key, field in utils.ensure_mapping_format(extractors).items():
                extracted_variable_mapping[key]= test_dict.get('variables').get(field.split('.')[1])
            self.session_context.update_session_variables(extracted_variable_mapping)
    ...
    def run_test(self, test_dict):
        ...
        print('test!!!!!',test_dict,time.strftime('%Y%m%d%H%M%S'))
